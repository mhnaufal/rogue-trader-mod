...
public class PFStandardShaderGUI : ShaderGUIBase
{
    private MaterialProperty m_MaterialType;
    private MaterialProperty m_VertexAnimationType;
    private MaterialProperty m_VertexInteraction;
    ...
    public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] properties)
    {
        ...
        m_VertexAnimationType = properties.FirstOrDefault(p => p.name == "_VertexAnimationType");
        m_VertexInteraction = properties.FirstOrDefault(p => p.name == "_VertexInteraction");
    ...
    private void UpdateVertexAnimKeywords()
    {
        var mat = m_MaterialEditor.target as Material;
        if (m_VertexInteraction.floatValue == 1)
        {
            mat.EnableKeyword("VERTEX_INTERACTION_ON");
        }
        else
        {
            mat.DisableKeyword("VERTEX_INTERACTION_ON");
        }

        if (!mat.IsKeywordEnabled("VERTEX_ANIMATION_ON"))
        {
            //mat.SetOverrideTag("DisableBatching", "False");
            mat.DisableKeyword("VERTEX_ANIMATION_COMPLEX_ON");
            mat.DisableKeyword("VERTEX_ANIMATION_SIMPLE_ON");
        }
        else
        {
            //mat.SetOverrideTag("DisableBatching", "True");
            if (m_VertexAnimationType.floatValue == 1)
            {
                mat.EnableKeyword("VERTEX_ANIMATION_COMPLEX_ON");
                mat.DisableKeyword("VERTEX_ANIMATION_SIMPLE_ON");
            }
            else
            {
                mat.DisableKeyword("VERTEX_ANIMATION_COMPLEX_ON");
                mat.EnableKeyword("VERTEX_ANIMATION_SIMPLE_ON");
            }
        }
    }
