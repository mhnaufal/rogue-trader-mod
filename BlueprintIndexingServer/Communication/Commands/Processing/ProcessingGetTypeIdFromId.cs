...
public static class ProcessingGetTypeIdFromId
{
    public static Command? Processing(Command command, FileDatabase.FileDatabase database)
    {
        ...
        return new Command { Type = CommandTypes.Result, Payload = PayloadTypeIdData.Create(database.IdToType(payloadData?.Id ?? "")).ToJson() };
    }
}