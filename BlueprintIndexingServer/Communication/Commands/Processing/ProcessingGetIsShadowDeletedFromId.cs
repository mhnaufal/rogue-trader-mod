...
public static class ProcessingGetIsShadowDeletedFromId
{
    public static Command? Processing(Command command, FileDatabase.FileDatabase database)
    {
        ...
        return new Command { Type = CommandTypes.Result, Payload = PayloadIsShadowDeletedData.Create(database.IdToIsShadowDeleted(payloadData?.Id ?? "")).ToJson() };
    }
}